...
function buildChartData(schedules: SchedulerRecord[], period: string): DataPoint[] {
  ...
  if (period === 'This week') {
    ...
    return days.map((day, i) => {
      ...
      const count = schedules.filter((s) => s.set_date === toDateStr(d)).length
      return { date: day, posts: count }
    })
  }
  ...
  return Array.from({ length: daysInMonth }, (_, i) => {
    ...
    const count = schedules.filter((s) => s.set_date === dateStr).length
    return { date: String(day), posts: count }
  })
}

function buildPlatformData(schedules: SchedulerRecord[]): PlatformDataPoint[] {
  ...
  return Object.entries(counts)
    .map(([key, count]) => ({
      platform: key,
      displayName: PLATFORM_DISPLAY[key] ?? key,
      count,
...
function computeMetrics(schedules: SchedulerRecord[]): MetricsData {
  ...
  const topPlatformKey =
    Object.entries(platformCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? ''
  const mostActive = PLATFORM_DISPLAY[topPlatformKey] ?? topPlatformKey ?? '—'

  return { total, completed, pending, successRate, totalMedia, mostActive }
}

function convertEpochToDaysHours(epochTimestamp: number): { days: number } {
  const difference = Date.now() - epochTimestamp * 1000
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const load = async (): Promise<void> => {
      try {
        ...
        const mine = (all || []).filter((s) => s.Instance_id === instanceId)
        setSchedules(mine)
      } catch {
        setSchedules([])
      }
  ...
  const platformData = buildPlatformData(schedules)
  const upcoming = schedules
    .filter((s) => s.isScheduled === 0)
    .sort((a, b) => {
      const da = new Date(`${a.set_date}T${a.set_time}`).getTime()
      const db = new Date(`${b.set_date}T${b.set_time}`).getTime()
      return da - db
    })
    .slice(0, 6)

  return (
    <div className="w-full h-full p-4 flex flex-col gap-4 max-w-full overflow-y-auto">
      {/* Metrics card */}
      ...
      <div className="grid grid-cols-12 gap-4 w-full">
        <ShiftCard upcoming={upcoming} />
        <div className="col-span-9 flex flex-col gap-4">
          <DashboardChart chartData={chartData} period={selectedPeriod} />
            ...
            <div>
              ...
              <p className="text-sm">
                {licenseExpiry.days > 0 ? (
                  <>Your license expired <span className="font-bold">{licenseExpiry.days} day{licenseExpiry.days !== 1 ? 's' : ''} ago</span>. Please renew it.</>
                ) : (
                  <>Your license will expire in <span className="font-bold">{licenseExpiry.days * -1} day{licenseExpiry.days * -1 !== 1 ? 's' : ''}</span>.</>
                )}
              </p>
            </div>
          </div>
