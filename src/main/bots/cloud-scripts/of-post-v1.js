...
for (const schedule of schedules) {
  ...
  await click('//button[contains(text(), "Post")]');
  ...
  if (schedule.set_price > 0) {
    ...
    if (freeFiles.length > 0) {
      ...
      await click('xpath=//*[@data-icon-name="icon-arrow-left"]/..');
      await click('xpath=//*[@data-icon-name="icon-done"]/..');
    }
  }

  await chooseFiles('xpath=//*[@id="attach_file_photo"]', paidFiles);
  await typeText('//div[@data-placeholder="Compose new post..."]', schedule.description_text + '\n' + schedule.signature);
  await click('//button[@at-attr="submit_post" and not(@disabled)]');
  await waitMs(5000);

  done(schedule.id);
  log('OnlyFans post complete: ' + schedule.id);
}
